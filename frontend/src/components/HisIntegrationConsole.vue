...
const form = ref({
  ...
  priority: 'NORMAL',
  items: [
    { hisDrugCode: 'HIS-DRUG-001', drugName: '阿莫西林胶囊', specification: '0.25g*24粒', quantity: 1, unit: '盒' },
    { hisDrugCode: 'HIS-DRUG-002', drugName: '布洛芬片', specification: '0.2g*20片', quantity: 2, unit: '盒' }
  ]
})

const statusLabels = { PENDING: '待发送', PROCESSING: '发送中', SENT: '已送达', FAILED: '发送失败' }
const eventLabels = {
  APPLICATION_RECEIVED: '申请单接收', APPLICATION_READY: '药品匹配完成',
...
const addItem = () => {
  form.value.items.push({ hisDrugCode: '', drugName: '', specification: '', quantity: 1, unit: '盒' })
}
...
.medicine-editor-row { display: grid; grid-template-columns: 26px 1fr 1.25fr 1fr 70px 70px 34px; gap: 7px; align-items: center; }.medicine-editor-row input, .medicine-editor-row select { width: 100%; padding: 9px 8px; }.line-number { width: 24px; height: 24px; display: grid; place-items: center; background: #e8eef1; font-size: 11px; font-weight: 800; }.remove-button { width: 32px; height: 32px; display: grid; place-items: center; border: 0; background: transparent; color: #a23b32; cursor: pointer; }.remove-button svg { width: 17px; }
...
@media (max-width: 680px) { .connection-band, .form-grid { grid-template-columns: 1fr; }.connection-band > svg { display: none; }.connection-band .online { grid-column: auto; }.medicine-editor-row { grid-template-columns: 26px 1fr 34px; }.medicine-editor-row input, .medicine-editor-row select { grid-column: 2; }.medicine-editor-row .remove-button { grid-column: 3; grid-row: 1; }.callback-row { grid-template-columns: 8px 1fr; }.callback-state { grid-column: 2; justify-items: start; } }
</style>
